...
def create_project(project: ProjectCreate):
    ...
    try:
        ...
        db.execute(
            "INSERT INTO projects (id, name, size_mw, deadline, budget, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (project_id, project.name, project.size_mw, project.deadline, project.budget, "active", now)
        )
        db.commit()
